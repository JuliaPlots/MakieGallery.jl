...
@block SebastianM_C ["3d"] begin
    ...
    @cell "Interactive Differential Equation" [lines, slider, animated, record, diffeq] begin
        ...
        sσ, oσ = AbstractPlotting.textslider(exp10.(-OME:0.001:OME), "σ", start = p0[1]);

        sρ, oρ = AbstractPlotting.textslider(exp10.(-OME:0.001:OME), "ρ", start = p0[2]);

        sβ, oβ = AbstractPlotting.textslider(exp10.(-OME:0.001:OME), "β", start = p0[3]);

        st, ot = AbstractPlotting.textslider(exp10.(-OME:0.001:OME), "tₘₐₓ", start = tspan0[end]);

        sr, or = AbstractPlotting.textslider(100:10000, "resolution", start = 2000);
        ...
        scene = vbox(hbox(sσ, sρ, sβ, st, sr), three)

        N = 100

        record(scene, @replace_with_a_path(mp4), LinRange(0, 40, N)) do ρᵢ
