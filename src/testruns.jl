...
const current_ref_version = Ref{String}("v0.5.1")
    ...
    @testset "Reference Image Tests" begin
        ...
        for folder in folders
            ...
            if isdir(folder) && isdir(joinpath(reference, basename(folder)))
                ...
                ref_media = filter(x-> x != "thumb.jpg", sort(readdir(ref_folder)))
                test_media = filter(x-> x != "thumb.jpg", sort(readdir(test_folder)))
